...
type ErrorFormProps = {
  onSubmit: (values: CreateErrorRequestBody) => void;
...
export default function ErrorForm({
  ...
  const {
    ...
    control,
    formState: { errors },
  } = useForm<CreateErrorRequestBody>({
    defaultValues: {
      title: "",
      resolutionTime: 0,
      errorMessage: "",
  ...
  const handleAddTechnology = () => {
    ...
    if (selectedTechnologies.includes(name)) {
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
      ...
      {/* エラー詳細 */}
      <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        <h2 className="mb-6 text-lg font-bold text-gray-900">エラー詳細</h2>
      ...
      <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        <div className="mb-6 flex items-center justify-between">
          <h2 className="text-lg font-bold text-gray-900">調査ステップ</h2>
        ...
        <div className="space-y-4">
          {fields.map((field, index) => (
            ...
            >
              ...
              <input
                type="hidden"
                {...register(`attempts.${index}.attemptNumber`, {
                  valueAsNumber: true,
                })}
              />
            </div>
